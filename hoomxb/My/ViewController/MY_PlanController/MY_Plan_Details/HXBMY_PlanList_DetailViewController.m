...
- (void) setUP {
    ...
    self.planDetailView.tipNoviceClickBlock = ^{
        HXBXYAlertViewController *alertVC = [[HXBXYAlertViewController alloc] initWithTitle:@"温馨提示" Massage:[NSString stringWithFormat:@"计划按照%.1lf%%计息，加息收益%@元将在计划退出时发放至您的账户",weakSelf.planViewModel.planModelDataList.expectedRate.floatValue,weakSelf.planViewModel.planModelDataList.expectedSubsidyInterestAmount] force:2 andLeftButtonMassage:nil andRightButtonMassage:@"确定"];
        alertVC.isHIddenLeftBtn = YES;
        alertVC.isCenterShow = YES;
        [weakSelf presentViewController:alertVC animated:YES completion:nil];
    };
